...
class MyLayout(Widget):
    ...
    def selected_object(self, objId):
        print('click', objId)
        response = requests.get("https://api.nasa.gov/neo/rest/v1/neo/" + str(objId) + "?api_key=eLLQHjpU93iCInMRF8Cvrm3x2wLoQrtDGsinjF48")
        responseDump = json.dumps(response.json(),sort_keys=True, indent=4)
        estDiameter = json.loads(responseDump)['estimated_diameter']['meters']
        # CAD = json.loads(responseDump)['close_approach_data']
        loadedDump = json.loads(responseDump)['orbital_data']
        ...
        # periDistLab.config(text = 'Perihelion (Closest) Distance (AU): ' + str(round(float(loadedDump['perihelion_distance']),2)))
        # periArgLab.config(text = 'Perihelion Argument (deg): ' + str(round(float(loadedDump['perihelion_argument']),2)))
        # periodLab.config(text = 'Orbital Period (Days): ' + str(round(float(loadedDump['orbital_period']),2)))
        # meanMotionLab.config(text = 'Mean Motion (deg/d): ' + str(round(float(loadedDump['mean_motion']),2)))
        # semiMajAxisLab.config(text = 'Semi Maj Ax. (AU): ' + str(round(float(loadedDump['semi_major_axis']),2)))
        # ascNodeLongLab.config(text = 'Ascending Node Longitude (deg): ' + str(round(float(loadedDump['ascending_node_longitude']),2)))
        # incLab.config(text = 'Inclination (deg): ' + str(round(float(loadedDump['inclination']),2)))
        # estDiaLab.config(text = 'Estimated Diameter (meters) (max/min): ' + str(round(estDiameter['estimated_diameter_max'],2)) + '/' + str(round(estDiameter['estimated_diameter_min'],2)))
            
    def search(self):
        startDate = self.ids.start_date_label.text
        endDate = self.ids.end_date_label.text
        response = requests.get("https://api.nasa.gov/neo/rest/v1/feed?start_date=" + startDate + "&end_date=" + endDate + "&api_key=eLLQHjpU93iCInMRF8Cvrm3x2wLoQrtDGsinjF48")
        responseDump = json.dumps(response.json(),sort_keys=True, indent=4)
        loadedDump = json.loads(responseDump)
        idList = []
        objectNames = []
        allObjects = loadedDump['near_earth_objects']
